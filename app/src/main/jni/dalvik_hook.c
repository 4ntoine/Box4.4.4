...
void* dalvik_hook(struct dexstuff_t *dex, struct dalvik_hook_t *h)
{
   // if (h->debug_me)
    ...
    __android_log_print(ANDROID_LOG_VERBOSE, "dvm", "dalvik_hook: class %s\n", h->clname);
    __android_log_print(ANDROID_LOG_VERBOSE, "dvm", h->method_sig);
    void *target_cls = dex->dvmFindLoadedClass_fnPtr(h->clname);
    //if (h->debug_me)
       // log("class = 0x%x\n", target_cls)

    __android_log_print(ANDROID_LOG_VERBOSE, "dvm", "class = 0x%x\n", target_cls);
    ...
    if (h->method_sig == "sched")
    {
        __android_log_print(ANDROID_LOG_VERBOSE, "dvm", "inside sched");
        h->method = dex->dvmFindVirtualMethodHierByDescriptor_fnPtr(target_cls, h->method_name, "(Landroid/content/Intent;Landroid/os/IBinder;ILandroid/content/pm/ActivityInfo;Landroid/content/res/Configuration;Landroid/content/res/CompatibilityInfo;ILandroid/os/Bundle;Ljava/util/List;Ljava/util/List;ZZLjava/lang/String;"
                "Landroid/os/ParcelFileDescriptor;Z)V");
        if (h->method == 0) {
            h->method = dex->dvmFindDirectMethodByDescriptor_fnPtr(target_cls, h->method_name, "(Landroid/content/Intent;Landroid/os/IBinder;ILandroid/content/pm/ActivityInfo;Landroid/content/res/Configuration;Landroid/content/res/CompatibilityInfo;ILandroid/os/Bundle;Ljava/util/List;Ljava/util/List;ZZLjava/lang/String;"
                    "Landroid/os/ParcelFileDescriptor;Z)V");
        }
    }
    else{
    ...
    __android_log_print(ANDROID_LOG_VERBOSE, "dvm", "%s(%s) = 0x%x\n", h->method_name, h->method_sig, h->method);
    ...
    if (h->method) {
        ...
        //  if (h->debug_me) {
        //log("nativeFunc %x\n", h->method->nativeFunc)
        __android_log_print(ANDROID_LOG_VERBOSE, "dvm", "nativeFunc %x\n", h->method->nativeFunc);

        //log("insSize = 0x%x  registersSize = 0x%x  outsSize = 0x%x\n", h->method->insSize, h->method->registersSize, h->method->outsSize)
        //  }
        __android_log_print(ANDROID_LOG_VERBOSE, "dvm", "insSize = 0x%x  registersSize = 0x%x  outsSize = 0x%x\n", h->method->insSize, h->method->registersSize, h->method->outsSize);
    ...
    //  if (h->debug_me) {
    //      log("shorty %s\n", h->method->shorty)
        __android_log_print(ANDROID_LOG_VERBOSE, "dvm", "shorty %s\n", h->method->shorty);

        //    log("name %s\n", h->method->name)
        __android_log_print(ANDROID_LOG_VERBOSE, "dvm", "name %s\n", h->method->name);
        //      log("arginfo %x\n", h->method->jniArgInfo)
        __android_log_print(ANDROID_LOG_VERBOSE, "dvm", "arginfo %x\n", h->method->jniArgInfo);

        //   }
        h->method->jniArgInfo = 0x80000000; // <--- also important
    //   if (h->debug_me) {
    //        log("noref %c\n", h->method->noRef)
        __android_log_print(ANDROID_LOG_VERBOSE, "dvm", "noref %c\n", h->method->noRef);
    //       log("access %x\n", h->method->a)
        __android_log_print(ANDROID_LOG_VERBOSE, "dvm", "access %x\n", h->method->a);
    //   }
    ...
    //       log("patched %s to: 0x%x\n", h->method_name, h->native_func)
        __android_log_print(ANDROID_LOG_VERBOSE, "dvm", "patched %s to: 0x%x\n", h->method_name, h->native_func);
...
//          log("could NOT patch %s\n", h->method_name)
        __android_log_print(ANDROID_LOG_VERBOSE, "dvm", "could NOT patch %s\n", h->method_name);
